...
export const load: PageServerLoad = async ({ fetch }) => {
  ...
  const [Academics, Admin, Essentials, Food, Hostel, Sports] = await Promise.all([
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_ACAD}`, options),
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_ADMIN}`, options),
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_ESSENTIAL}`, options),
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_FOOD}`, options),
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_HOSTEL}`, options),
    fetch(`${PUBLIC_DATA_SOURCE_BASE}${PUBLIC_DATA_SOURCE_AMENITY_SPORT}`, options),
  ])

  const [AcademicsData, AdminData, EssentialsData, FoodData, HostelData, SportsData] = await Promise.all([
